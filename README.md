# expense-tracker
Practice project of expense tracker written in javascript, which can read, create, update and delete data stored in an external JSON file through APIs.
Here's how it looks:
...
To start the json-server as a backend run the command ```npx json-server -p 3500 -w data/sb.json``` on a seperate terminal instance.
Other backend APIs can also be set up.
